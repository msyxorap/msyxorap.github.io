...
## After you edit: bump the cache version

`index.html` links its CSS and JS with a `?v=N` query string. Browsers cache
those files aggressively, so if you change `content.js` or `style.css` and
do not bump `N`, visitors (and you) keep seeing the old version and the edit
looks like it did nothing.

Increment all four `?v=` numbers in `index.html` in the same commit as any
change to a file under `assets/`.
